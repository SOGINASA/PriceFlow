...
По умолчанию (в т.ч. авто-сид при старте, см. app.py) создаётся ТОЛЬКО
необходимая учётная запись — администратор. Синтетические
...
# Только admin: вход в систему идёт через POST /api/admin/login,
# который принимает роли admin|partner, поэтому обычные user-аккаунты
...
def seed_all():
    """Идемпотентно создать ТОЛЬКО необходимые учётные записи (админ).
...
def seed_demo():
    ...
    """
    users = _seed_users()                    # гарантируем админ-аккаунт (нужен для входа)
