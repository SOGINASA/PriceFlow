  ...
  return (
    <section id="search" className="px-6 pt-[70px] pb-20 max-w-[1000px] mx-auto">
      <div ref={headRef} style={revealStyle(headVis)} className="text-center max-w-[640px] mx-auto mb-9">
        <div className="text-[13px] font-bold tracking-[.06em] uppercase" style={{ color: "#8B88FF" }}>{tr.search.eyebrow}</div>
        <h2 className="font-display font-bold mt-[14px]" style={{ fontSize: "clamp(28px,4vw,42px)", letterSpacing: "-.03em" }}>{tr.search.title}</h2>
      </div>
      <div ref={boxRef} style={revealStyle(boxVis)} className="rounded-3xl bg-white/[.03] border border-white/[.08] p-[18px]" >
        <ClinicSearch
