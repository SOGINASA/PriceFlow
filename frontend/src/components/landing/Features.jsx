...
function FeatureCard({ icon, title, desc, delay }) {
  ...
  return (
    <div
      ref={ref}
      style={{ ...revealStyle(vis, delay), background: "linear-gradient(180deg,rgba(255,255,255,.026),rgba(255,255,255,.004))" }}
      className="group relative p-[34px] px-[30px] rounded-[22px] border border-white/[.07] transition-all duration-300 hover:-translate-y-1 hover:border-primary/45"
    >
      <div className="text-lav mb-6" style={{ filter: "drop-shadow(0 4px 14px rgba(94,92,230,.35))" }}>
  ...
  return (
    <section id="features" className="px-6 pt-[70px] pb-20 max-w-[1200px] mx-auto">
      <div ref={headRef} style={revealStyle(headVis)} className="max-w-[680px] mx-auto mb-[50px] text-center">
        <div className="text-[13px] font-bold tracking-[.06em] uppercase" style={{ color: "#8B88FF" }}>{tr.features.eyebrow}</div>
        <h2 className="font-display font-bold mt-[14px] whitespace-pre-line" style={{ fontSize: "clamp(30px,4.5vw,46px)", letterSpacing: "-.03em", lineHeight: 1.08 }}>{tr.features.title}</h2>
      </div>
