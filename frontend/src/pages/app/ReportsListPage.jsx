...
// ---------- Список отчётов ----------
// Сетка карточек всех обработанных отчётов. Клик по карточке → детальная
    ...
    <section className="flex flex-col gap-4 animate-fade-up">
      <div className="flex items-center justify-between">
        <div className="text-sm font-semibold text-ink/70">
          Все отчёты <span className="text-ink/40">({REPORTS.length})</span>
        </div>
        <Link
          to="/app/upload"
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-[14px]">
        {REPORTS.map((r, i) => (
          ...
          >
            ...
            <div className="text-[12.5px] text-ink/45 mt-1">{r.date}</div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-white/[0.06]">
              ...
              <div>
                <div className="font-display font-bold text-[18px]">{r.items.toLocaleString("ru-RU").replace(/,/g, " ")}</div>
