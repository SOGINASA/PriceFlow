...
export default function AnalyzingPage() {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        setTimeout(() => navigate("/app/report"), 900);
      }
    }, 95);
  ...
  return (
    <section className="flex flex-col items-center gap-2 pt-[6px] animate-fade-up">
      {/* ---------- Кольцо прогресса ---------- */}
      <div className="relative w-full max-w-[680px] rounded-[26px] border border-white/[.08] overflow-hidden p-[38px] px-[30px]" style={{ background: "linear-gradient(180deg,rgba(20,20,28,.7),rgba(12,12,18,.6))", boxShadow: "0 30px 90px rgba(0,0,0,.5)" }}>
        <div className="absolute pointer-events-none rounded-full" style={{ top: "-30%", left: "50%", width: 420, height: 420, marginLeft: -210, background: "radial-gradient(circle,rgba(94,92,230,.3),transparent 65%)", filter: "blur(50px)" }} />
        <div className="relative flex flex-col items-center">
          <div className="relative w-[200px] h-[200px]">
            <svg viewBox="0 0 200 200" className="w-full h-full">
              <circle cx="100" cy="100" r="88" fill="none" stroke="rgba(255,255,255,.07)" strokeWidth="5" />
              <circle cx="100" cy="100" r="88" fill="none" stroke="url(#anGrad)" strokeWidth="6" strokeLinecap="round" strokeDasharray={RING_CIRC} strokeDashoffset={ringOffset} transform="rotate(-90 100 100)" style={{ transition: "stroke-dashoffset .2s linear" }} />
              <circle cx="100" cy="100" r="58" fill="none" stroke="#7E7BFF" strokeWidth="1.5" strokeDasharray="5 11" opacity=".6" className="animate-spin-slow" style={{ transformOrigin: "100px 100px" }} />
          ...
          <div className="flex flex-wrap gap-[9px] justify-center mt-6">
            {PHASES.map((label, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 px-[14px] py-[9px] rounded-[11px] border transition-all" style={{ background: active || done ? "rgba(94,92,230,.14)" : "rgba(255,255,255,.04)", borderColor: active || done ? "rgba(94,92,230,.35)" : "rgba(255,255,255,.08)" }}>
                  <span className="grid place-items-center w-4 h-4 rounded-full" style={{ background: done ? "#30D158" : active ? "#6E8BFF" : "rgba(245,245,247,.25)" }}>
                    {done && (
                  ...
                  <span className="text-[13px] font-semibold" style={{ color: active || done ? "#F5F5F7" : "rgba(245,245,247,.65)" }}>{label}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* ---------- Журнал обработки ---------- */}
      <div className="w-full max-w-[680px] mt-4 rounded-[18px] border border-white/[.07] overflow-hidden" style={{ background: "rgba(10,10,15,.7)" }}>
        <div className="flex items-center gap-[9px] px-4 py-[11px] border-b border-white/[.06]">
          <span className="w-2 h-2 rounded-full animate-pulse-dot" style={{ background: "#6E8BFF" }} />
          <span className="text-[12.5px] font-semibold text-ink/55 font-display">Журнал обработки</span>
        </div>
        <div className="px-4 py-[14px] flex flex-col gap-[7px] h-[148px] overflow-hidden text-[12.5px]" style={{ fontFamily: "ui-monospace,SFMono-Regular,Menlo,monospace" }}>
          {logLines.map((line, i) => (
            <div key={`${line}-${i}`} className="flex items-center gap-[9px] text-ink/70 animate-log-in">
              <span style={{ color: "#5BE892" }}>✓</span>
              <span>{line}</span>
            </div>
          ))}
        </div>
